...
import {
    ...
    searchFieldLogs,
    updateFieldLogs
} from "../database/field-logs-data-store";
...
router.post('/add', upload.fields([{ name: 'img', maxCount: 1 },]), async (req, res) => {
    ...
    try {
        await addFieldLogs(logs);
        res.send('Field Logs added successfully');
    } catch (error) {
        console.error(error);
        res.status(500).send('Error adding field logs');
    }
});
...
router.delete('/delete/:log_code', async (req, res) => {
    ...
    try {
        await deleteFieldLogs(log_code);
...
router.get('/get', async (req, res) => {
    try {
        const logs = await getAllFieldLogs();
        res.json(logs);
    } catch (error) {
        console.error(error);
...
router.put('/update/:log_code', upload.fields([{ name: 'img', maxCount: 1 },]), async (req, res) => {
    ...
    try {
        await updateFieldLogs(log_code, logs);
...
router.get('/search/:log_code', async (req, res) => {
    const log_code = req.params.log_code;
    try {
        const logs = await searchFieldLogs(log_code);
        res.json(logs);
    } catch (error) {
        console.error(error);
